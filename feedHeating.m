function[state,WmovAdd,Wop,pumpExtractLossEn,pumpExtractLossEx,turbineLossEn,turbineLossEx,diffHeatingExergy,indexDeaerator]=feedHeating(state,pmax,eta_siP,eta_siT,eta_mec,nF,nR,dTpinch)
...
for index=1:nF
    ...
    if deaeratorON && nF > 2 && Tsat>120 %condition supp necessaire ???  
        ...
        [state(5+2*nR,index), ~] = deaerator(state(4+2*nR,index),Tsat);
        %state(6+2*nR,indexDeaerator)=0;%state that correspond to the out of the valve. There is no valve so we put this equals to 0
    else
    ...
    for index=1:(nF+1)
        ...
        elseif index == indexDeaerator+1
            ...
            [state(11+2*nR,index),Wop,pumpExtractLossEn, pumpExtractLossEx]=extractionPump(state(5+2*nR,indexDeaerator),0.5,pmax,eta_siP,eta_mec);
